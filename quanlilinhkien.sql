...
-- 5. Tạo lại Procedure: Lấy chi tiết tài khoản
...
BEGIN
    SET NOCOUNT ON;
    SELECT 
        u.MaNguoiDung AS UserID,       -- Map sang tên tiếng Anh để code C# không bị lỗi
        u.HoTen AS FullName,
        u.Email,
        u.SoDienThoai AS Phone,
        u.DiaChi AS Address,
        u.MatKhauHash AS PasswordHash, -- Cột mật khẩu
        u.MaVaiTro AS RoleID,
        r.TenVaiTro AS RoleName,
        u.TrangThai AS IsActive,
...
CREATE PROCEDURE sp_UpdateUserFull
    @UserID INT,
    @HoTen NVARCHAR(100),
    @SoDienThoai VARCHAR(15),
    @DiaChi NVARCHAR(255),
...
AS
BEGIN
    -- Chặn sửa quyền của Admin cuối cùng
    IF (@RoleID != 1) AND NOT EXISTS (SELECT 1 FROM NguoiDung WHERE MaVaiTro = 1 AND MaNguoiDung != @UserID AND TrangThai = 1)
    BEGIN
        SELECT N'Lỗi: Không thể giáng cấp Quản trị viên duy nhất.' AS Message, 0 AS Success;
        RETURN;
    END
    ...
    SELECT N'Cập nhật thông tin thành công!' AS Message, 1 AS Success;
END;
GO

-- . Tạo lại Procedure: Cập nhật Vai trò & Trạng thái
CREATE PROCEDURE sp_UpdateUserRoleAndStatus
    @UserID INT,
    @NewRoleID INT,
    @IsActive BIT
AS
BEGIN
    ...
    -- Kiểm tra logic: Không cho xóa Admin cuối cùng
    -- Giả sử RoleID = 1 là Admin
    IF (@NewRoleID != 1) AND NOT EXISTS (SELECT 1 FROM NguoiDung WHERE MaVaiTro = 1 AND MaNguoiDung != @UserID AND TrangThai = 1)
    BEGIN
        SELECT N'Lỗi: Đây là Quản trị viên duy nhất. Không thể thay đổi quyền.' AS Message, 0 AS Success;
        RETURN;
    END
    ...
    END TRY
    BEGIN CATCH
        SELECT N'Lỗi: ' + ERROR_MESSAGE() AS Message, 0 AS Success;
    END CATCH
END;
GO